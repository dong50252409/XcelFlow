...
import { flatbuffers, flexbuffers, BaseData_Fbs, cacheStrRes, cacheObjRes, FbsData, FbsDataList } from './index';
{{- end -}}`
...
export class {{ $configName }} extends BaseData_Fbs<Cfg{{ $configName }}> {
    ...
    public getFbsDataList(data: ArrayBuffer): FbsDataList<Cfg{{ $configName }}> {
        return new Cfg{{ $configName }}DetailList(data);
    }
...
{{- $configName := .Table.ConfigName | toUpperCamelCase -}}
export class Cfg{{ $configName }}DetailList extends FbsDataList<Cfg{{ $configName }}> {
...
export class Cfg{{ $configName }} extends FbsData {
    ...
    __toObject(i: number): unknown {
        ...
        if (offset) {
            ...
            return flexbuffers.toObject(array);
        }
        return null
    }
    ...
    public get {{ $field.Name | toLowerCamelCase }}(): {{ $field.Type }} {
        ...
        return offset ? this.bb!.{{ $field.Type.MethodStr }}(this.bb_pos + offset) : {{ $field.Type.DefaultValue }};
        {{- end }}
    }
    {{ end }}
}
...
{{ range $_, $macro := .Table.GetMacros }}
// {{ $macro.KeyField.Comment }}
export enum {{ $macro.MacroName | toUpperCamelCase }}Enum { 
	{{- range $_, $macroDetail := $macro.DetailList }}
    /** {{ $macroDetail.Comment }} */
    {{ $macroDetail.Key | toUpperSnakeCase }} = {{ $macroDetail.Value | $macro.ValueField.Convert }},
	{{- end }}
}
{{ end }}
{{- end -}}`
