...
-record({{ .Table.ConfigName }}, {
    ...
    {{ $field.Name | toSnakeCase }} = {{ $field.DefaultValueStr }} :: {{ $field.Type }}{{ if lt $index $lastIndex }},{{ end }}	% {{ $field.Comment }}
    {{- end }}
}).
{{- end -}}
{{- end -}}`
...
-define({{ $macroDetail.Key | toUpperSnakeCase }}, {{ $macroDetail.Value | $macro.ValueField.Convert }}).	  % {{ $macroDetail.Comment }}
{{ end }}
{{- end -}}
{{- end -}}`
